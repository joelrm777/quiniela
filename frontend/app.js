...
function switchPhase(phase) {
    ...
    if (phase === 'grupos') {
        // Auto-detect the best jornada (today's or next upcoming)
        autoSelectBestRound();
    } else {
        updateMatchesHeader();
        loadActiveMatches();
...
// Auto-selects the jornada that best matches today's date:
// 1. Jornada with matches TODAY   → top priority
// 2. Jornada with the next upcoming match (future) → second
// 3. Jornada with the most recent past match       → fallback
async function autoSelectBestRound() {
    const todayStr = new Date().toLocaleDateString('sv');
    const now = new Date();

    try {
        // Fetch all 3 jornadas in parallel (no group filter)
        const [r1, r2, r3] = await Promise.all([
            fetch(`${API_BASE}/matches?phase=grupos&round=1`).then(r => r.json()),
            fetch(`${API_BASE}/matches?phase=grupos&round=2`).then(r => r.json()),
            fetch(`${API_BASE}/matches?phase=grupos&round=3`).then(r => r.json()),
        ]);
        const jornadas = [
            { round: 1, matches: r1 },
            { round: 2, matches: r2 },
        ...
        if (!jornadas.length) {
            // No matches at all → default to round 1
            setRound(1);
            return;
        }

        // 1. Any jornada with a match today?
        const todayJornada = jornadas.find(j =>
            j.matches.some(m => new Date(m.match_time).toLocaleDateString('sv') === todayStr)
        );
        if (todayJornada) { setRound(todayJornada.round); return; }

        // 2. Jornada with the closest FUTURE match
        let bestFuture = null, bestFutureMs = Infinity;
        for (const j of jornadas) {
            for (const m of j.matches) {
                const ms = new Date(m.match_time) - now;
                if (ms > 0 && ms < bestFutureMs) { bestFutureMs = ms; bestFuture = j.round; }
            }
        }
        if (bestFuture !== null) { setRound(bestFuture); return; }

        // 3. Jornada with the most recent PAST match
        let bestPast = null, bestPastMs = -Infinity;
        for (const j of jornadas) {
            for (const m of j.matches) {
                const ms = new Date(m.match_time) - now;
                if (ms <= 0 && ms > bestPastMs) { bestPastMs = ms; bestPast = j.round; }
            }
        }
        setRound(bestPast ?? jornadas[jornadas.length - 1].round);
    } catch {
        setRound(1);
    }
...
// Change own password
async function handleOwnChangePassword(event) {
    event.preventDefault();
    const msgEl      = document.getElementById('own-change-password-msg');
    const newPwd     = document.getElementById('own-new-password').value;
    const confirmPwd = document.getElementById('own-confirm-password').value;

    msgEl.style.display = 'none';

    if (newPwd !== confirmPwd) {
        msgEl.className = 'alert alert-danger';
        msgEl.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> Las contraseñas no coinciden.';
    ...
    try {
        const res = await fetch(`${API_BASE}/auth/change-password`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                current_password: document.getElementById('own-current-password').value,
                new_password: newPwd
            })
        });
        const result = await res.json();
        if (res.ok) {
            ...
            event.target.reset();
            setTimeout(() => closeModal('modal-change-password'), 1800);
        } else {
            msgEl.className = 'alert alert-danger';
...
async function loadActiveMatches() {
    ...
    try {
        ...
        // ── Separate matches into 3 groups by date ──────────────────────
        const todayStr = new Date().toLocaleDateString('sv');   // "YYYY-MM-DD" local

        const todayMatches    = matches.filter(m => new Date(m.match_time).toLocaleDateString('sv') === todayStr);
        const upcomingMatches = matches.filter(m => new Date(m.match_time).toLocaleDateString('sv') >  todayStr);
        const pastMatches     = matches.filter(m => new Date(m.match_time).toLocaleDateString('sv') <  todayStr);

        // Helper: build and append a full-width section divider
        function appendDivider(label, icon, cssClass) {
            const div = document.createElement("div");
            div.className = `matches-day-divider ${cssClass}`;
            div.style.gridColumn = "1 / -1";
            div.innerHTML = `<i class="fa-solid ${icon}"></i> ${label}`;
            grid.appendChild(div);
        }

        // Helper: build and append a single match card
        function appendMatchCard(m) {
            ...
            let badgeHtml = "";
            if (isFinished)     badgeHtml = `<span class="card-header-badge badge-finished">Finalizado</span>`;
            else if (isClosed)  badgeHtml = `<span class="card-header-badge badge-locked">Cerrado</span>`;
            else                badgeHtml = `<span class="card-header-badge badge-open">Abierto</span>`;

            let vsContent = `<span class="vs-text">VS</span>`;
            if (isFinished) vsContent = `<div class="real-score-box">${m.home_score} - ${m.away_score}</div>`;
            ...
            card.innerHTML = `
                ...
                <div class="match-meta">
                    ...
                    ${(!isFinished && !isClosed)
                        ...
                        : (isClosed && !isFinished
                            ? `<span class="text-muted text-sm"><i class="fa-solid fa-lock"></i> Bloqueado para cambios</span>`
                            : `<span class="text-gold text-sm"><i class="fa-solid fa-circle-check"></i> Resultado Oficial</span>`)
                    }
                </div>
                ...
                <div class="match-actions">
                    ...
                    ${(isClosed || isFinished)
                        ? `<button class="btn btn-view-predictions" onclick="openUserMatchPredictionsModal(${m.id})">
                               <i class="fa-solid fa-eye"></i> Ver Pronósticos del Grupo
                           </button>` : ''
                    }
                </div>
            `;
        ...
        // ── Render order: Hoy → Próximos → Anteriores ───────────────────
        if (todayMatches.length > 0) {
            const todayLabel = new Date().toLocaleDateString('es-ES', { weekday:'long', day:'numeric', month:'long' });
            appendDivider(`Hoy — ${todayLabel}`, 'fa-calendar-day', 'divider-today');
            todayMatches.forEach(appendMatchCard);
        }

        if (upcomingMatches.length > 0) {
            appendDivider('Próximos Partidos', 'fa-calendar', 'divider-upcoming');
            upcomingMatches.forEach(appendMatchCard);
        }

        if (pastMatches.length > 0) {
            appendDivider('Partidos Anteriores', 'fa-clock-rotate-left', 'divider-past');
            pastMatches.forEach(appendMatchCard);
        }

        if (!todayMatches.length && !upcomingMatches.length && !pastMatches.length) {
            grid.innerHTML = `<div class="text-center text-muted" style="grid-column: 1/-1; padding: 40px;"><i class="fa-solid fa-calendar-minus fa-2x"></i><p style="margin-top:10px;">No hay partidos cargados para esta jornada/grupo.</p></div>`;
        }
...
async function openMatchPredictionsModal(matchId, endpoint) {
    const apiEndpoint = endpoint || `${API_BASE}/admin/matches/${matchId}/predictions`;
    ...
    const isAdminCall = apiEndpoint.includes('/admin/');
    ...
    try {
        ...
        if (thead) {
            thead.innerHTML = `<th>#</th><th>Usuario</th><th>Pronóstico</th>`;
        }

        const preds = data.predictions;
        if (!preds.length) {
            tbody.innerHTML = `<tr><td colspan="3" class="text-center text-muted">Ningún usuario ha registrado pronóstico para este partido.</td></tr>`;
            return;
        }
        ...
        // Only show export button for admin calls
        if (isAdminCall) exportBtn.style.display = 'inline-flex';
    } catch(e) {
        tbody.innerHTML = `<tr><td colspan="7" class="text-danger text-center">Error al cargar pron\u00f3sticos.</td></tr>`;
    }
}

// User-facing predictions viewer (closed/finished matches only)
function openUserMatchPredictionsModal(matchId) {
    openMatchPredictionsModal(matchId, `${API_BASE}/matches/${matchId}/predictions`);
}
