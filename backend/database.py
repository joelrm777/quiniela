...
def get_db_connection():
    # Ensure sslmode is set (required by Supabase / most hosted PostgreSQL services)
    url = DATABASE_URL
...
def init_db():
    ...
    # ── Migraciones para tablas existentes en producción ──────────────────
    # PostgreSQL soporta ADD COLUMN IF NOT EXISTS desde v9.6
    migrations = [
        "ALTER TABLE users   ADD COLUMN IF NOT EXISTS is_active   INTEGER DEFAULT 1",
        "ALTER TABLE matches ADD COLUMN IF NOT EXISTS phase       TEXT    DEFAULT 'grupos'",
        "ALTER TABLE matches ADD COLUMN IF NOT EXISTS group_name  TEXT    DEFAULT ''",
