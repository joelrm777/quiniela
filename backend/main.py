...
from models import UserAuth, PredictionInput, MatchInput, MatchResultInput, UserUpdateInput, UserPasswordInput, ChangeOwnPasswordInput, ScoringRulesUpdateInput, MatchUpdateInput
...
app = FastAPI(title="Quiniela Mundial 2026 API", lifespan=lifespan)
...
# Global handler: turn DB / unhandled errors into a proper JSON 503 response
import psycopg2
@app.exception_handler(psycopg2.Error)
async def db_error_handler(request: Request, exc: psycopg2.Error):
    return JSONResponse(
        status_code=503,
        content={"detail": "El servicio de base de datos no está disponible. Intente de nuevo en unos segundos."}
    )

@app.exception_handler(Exception)
async def generic_error_handler(request: Request, exc: Exception):
    # Only catch truly unhandled exceptions (not HTTPException)
    if isinstance(exc, HTTPException):
        raise exc
    return JSONResponse(
        status_code=500,
        content={"detail": "Error interno del servidor. Intente de nuevo."}
    )
...
def get_scoring_rules_dict(cursor) -> dict:
    _default = {"outcome_correct": 3, "exact_score": 2, "home_goals": 1, "away_goals": 1}
    try:
        cursor.execute("SELECT rule_key, points FROM scoring_rules")
        rows = cursor.fetchall()
        return {row["rule_key"]: row["points"] for row in rows} if rows else _default
    except Exception:
        return _default
...
def login(user: UserAuth, response: Response):
    ...
    cursor = conn.cursor()
    try:
        # Intentar SELECT con is_active; si la columna no existe en producción
        # hacer rollback y reintentar sin ella (compatibilidad hacia atrás)
        try:
            cursor.execute(
        ...
        is_active = db_user.get("is_active", 1)
        if not is_active:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
def change_own_password(data: ChangeOwnPasswordInput, user = Depends(get_current_user)):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
...
def get_match_predictions_public(match_id: int, user = Depends(get_current_user)):
    """Returns predictions for a closed/finished match. Visible to all authenticated users."""
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM matches WHERE id = %s", (match_id,))
        ...
        if not match:
            raise HTTPException(status_code=404, detail="Partido no encontrado.")
        if match["status"] not in ("closed", "finished"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Los pronósticos solo son visibles cuando el partido está cerrado o finalizado."
            )
